...
public class ByteService {
	 ...
	 * 
	 * 바이트(Byte)기반 스트림 - 1byte 단위로 데이터를 입 / 출력 하는 스트림 - 최상위 클래스 : InputStream /
	 * OutputStream - 문자열, 이미지, 영상, 오디오, pdf 등 모든것을 입 / 출력 할 수 있음 (단 , 통로가 좁다보니 속도가
	 * 좀 느림, 1byte 초과인 문자같은 경우 깨질 우려가 있다)
	 * 
	 * 
	 */
...
//			}
	...
	public void bufferedFileByteOutput() {
		/*
		 * 버퍼(Buffer) 란 ? - 데이터를 모아두는 공간 , 바구니 같은 메모리 공간
		 * 
		 * BufferedOutputStream / BufferedInputStream 출력용 보조 스트림(바이트) / 입력용 보조 스트림(바이트)
		 * 
		 * - FileOutputStream 은 1바이트 씩 데이터를 입출력 -> 통로가 좁아서 데이터 입출력 시 속도가 느림 -> 보조스트림 이용
		 * 시 버퍼에 입출력할 내용을 모아서 한번에 입출력 ! -> 기반 스트림(FileOutputStream)을 이용하는 횟수, 시간이 줄어듬 ->
		 * 성능 , 시간 효율 향상
		 * 
		 * 
		 */
...
//			}
	...
	public void fileByteInput() {
		...
		try {
			...
			// java.io.FileNotFoundException: \io_test\20251014\노래가사.txt (지정된 파일을 찾을 수 없습니다)
			// FileInputStream 이용시 해당 경로에 반드시 실제파일이 존재해야함
	...
	public void fileCopy() {
		...
		try {
			...
			// 원본에서 읽어온 내용 bytes 를 bos 를 이용해서 출력(쓰기)
			bos.write(bytes);
