...
public class FileService {
	...
	public void method1() {
		...
		// 명시적으로 맨앞에 C: 또는 D: 라고 작성 시 해당 드라이브에 폴더 생성됨
		// == 절대경로 작성법

		// "io_test/20251014" 작성 시 해당 프로젝트가 현재 위치 !!!
		// 해당 프로젝트(12_io) 안에 io_test/20251014 이 생성됨
	...
	public void method2() {
		...
		if (!file.exists()) { // 존재하지않으면
			...
			// -> IOException 는 반드시 예외처리를 해야하는 Checked Exception
			// -> 예외 처리 필수 !
	...
	public void method3() {
		...
		for(File f : files) {
			...
			// 매개변수 long 값을 지정된 패턴 형식으로 변환하여 문자열 형태로 반환 메서드
			...
			if(f.isDirectory()) size = ""; // 폴더라면 사이즈 빈문자열 변경
