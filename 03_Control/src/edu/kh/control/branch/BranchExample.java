...
public class BranchExample {
	...
	public void RSPGame() {
		...
		// 컴퓨터 : 가위 바위 보 Math.random() : 0.0 ~ 1.0 사이 난수 생성
		// 1 ~ 3 사이 난수 생성
		// 1이면 가위, 2이면 바위, 3이면 보 지정
		// 컴퓨터와 플레이어 가위바위보 판별
		// 플레이어 승! / 졌습니다 ㅠㅠ / 비겼습니다
		// 매판마다 - 현재기록 : 몇승 몇무 몇패 출력
		...
		for(int i = 1; i <= round; i++) {
			...
			// Math.randon() : 0.0 ~ 1.0 사이
			// 0.0 <= X < 1.0
			// 0.0 <= X * 3 < 3.0
			// 1.0 <= X * 3 +1 < 4.0
			// 1 <= X < 4
			// 1이상 4 미만 정수 랜덤출력 -> 1 2 3
...
//			String com2 = "";
//			String com3; 선언만 이루어짐 -> com3이름의 상자만 메모리에 생성
//			값이 전혀 없음(초기화가 안된 상태)
			switch(random) {
			case 1 : com = "가위"; break;
			case 2 : com = "바위"; break;
	...
	public void ex5() {
		...
		for(int i = 1; i <= round; i++) {
			...
			boolean win3 = player.equals("보") && com.equals("주먹");
