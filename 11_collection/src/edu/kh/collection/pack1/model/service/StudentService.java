...
public class StudentService {
	...
	// 학생 정보를 저장할 List 생성
	private List<Student> studentList = new ArrayList<>(); // new ArrayList 시험나올확률 매우높음 무조건 넣어야함
	...
	public String addStudent() {
		...
		// UncheckedException : 강제처리 X, 예외 발생하면 
...
//		if(studentList.size() == 0)
		...
		// 반복문을 이용하여 studentList 에 있는 전체 학생 정보 출력
...
//		}
	...
	public String updateStudent() {
		...
		// E List.set(int index, E e) : 주어진 index 번째에 주어진 객체로 덮어쓰기(수정함)
		// 기존의 객체를 반환
	 ...
	 * 
	 * - 검색할 이름을 입력받아 studentList 에서 꺼내온
	...
	public void searchName2() {
		...
		System.out.print("검색할 이름(문자열) 입력 : ");
		String input = sc.next();
		...
		for(Student std : studentList) {
			...
			// String 에 문자열이 포함되어있으면 true / false
			if(std.getName().contains(input)) {
				//std.getName 에 input 이 포함되어 있느냐?
					System.out.println(std);
					flag = false;
			}
	...
	* Student 에 Comparable 인터페이스를 상속받아
	...
	private void sortByName() {
		Collections.sort(studentList, new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				...
				// (String 이 Comparable 을 상속받아 재정의해둔
